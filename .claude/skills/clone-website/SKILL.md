...
3. Verify the base project builds: `pnpm build`. The Vite + React 19 + shadcn/ui + Tailwind v4 scaffold should already be in place. If not, tell the user to run `pnpm install` first.
...
Every builder agent must verify `pnpm exec tsc --noEmit` passes before finishing. After merging worktrees, you verify `pnpm build` passes. A broken build is never acceptable, even temporarily.
...
**Fonts** — Inspect `<link>` tags for Google Fonts or self-hosted fonts. Check computed `font-family` on key elements (headings, body, code, labels). Document every family, weight, and style actually used. Add them to `src/index.css` as `@import` rules and update `--font-sans` / `--font-display` CSS variables.

**Colors** — Extract the site's color palette from computed styles across the page. Update `src/index.css` with the target's actual colors in the `:root` and `.dark` CSS variable blocks. Map them to shadcn's token names (background, foreground, primary, muted, etc.) where they fit. Add custom properties for colors that don't map to shadcn tokens.

**Favicons & Meta** — Download favicons, apple-touch-icons, OG images to `public/`. Update `index.html` meta tags.
...
1. **Update fonts** in `src/index.css` to match the target site's actual fonts via `@import`
2. **Update `src/index.css`** with the target's color tokens, spacing values, keyframe animations, utility classes, and any **global scroll behaviors** (Lenis, smooth scroll CSS, scroll-snap on body)
...
After all sections are built and merged, wire everything together in `src/App.tsx`:
...
- Implement page-level behaviors: scroll snap, scroll-driven animations, intersection observers, smooth scroll (Lenis etc.)
...
- **Don't extract only the default state.** Click every tab; scroll past every trigger threshold.
...
- **Don't build everything in one monolithic commit.** The whole point of this pipeline is incremental progress.
- **Don't reference docs from builder prompts.** Each builder gets the CSS spec inline — no external doc references.
- **Don't skip asset extraction.** Without real images, videos, and fonts, the clone will always look fake.
- **Don't give a builder agent too much scope.** If the builder prompt is getting long, split the section.
- **Don't bundle unrelated sections into one agent.** One section = one agent (or less for very complex sections).
- **Don't skip responsive extraction.** Test at 1440, 768, and 390 during extraction.
- **Don't forget smooth scroll libraries.** Check for Lenis (`.lenis` class) or Locomotive Scroll.
- **Don't dispatch builders without a spec file.** The spec file is non-negotiable.
